...
const CategoryAllProducts = () => {
    ...
    const fetchCategoryAndSubcategories = async () => {
      try {
        const categoryResponse = await axios.get(
          `http://localhost:3005/api/single-category/${categoryId}`
        );
        setCategoryName(categoryResponse.data.category_name);

        const subcategoryResponse = await axios.get(
          `http://localhost:3005/api/all-subcategories/${categoryId}`
        );
      ...
      const fetchProducts = async () => {
        try {
          const response = await axios.get(
            `http://localhost:3005/api/products-by-subcategory/${selectedSubcategory._id}`
          );
  ...
  const addToCart = async (product) => {
    try {
        ...
        console.error("Error adding product to the cart:", error.response?.data || error.message);
