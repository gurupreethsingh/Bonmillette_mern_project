    ...
    const fetchUserDetails = async () => {
      try {
        const response = await axios.get(
          `http://localhost:3006/api/user/${id}`, // Correct API endpoint
          {
            headers: {
  ...
  const handleRoleUpdate = async () => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:3006/api/update-user-role/${id}`, // API endpoint for updating role
        { role: newRole },
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
      ...
      <div className="bg-white p-6 rounded-lg">
        ...
        <div className="mb-4">
          ...
          >
            ...
            <option value="outlet">Outlet / Bon Millette Factory</option>
